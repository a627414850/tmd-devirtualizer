...
IR::exprptr apply_xchg(IR::exprptr expression, const std::list<std::shared_ptr<IR::Instruction>>& xchg_list)
{
	...
	{
		case IR::expression_type::register_:
		case IR::expression_type::variable:
		{
		...
		case IR::expression_type::memory:
		{
		...
		case IR::expression_type::immediate:
		{
			break;
		}
		case IR::expression_type::unary_op:
		{
			expression->set_operand(0, apply_xchg(expression->get_operand(0), xchg_list));
			break;
		}
		case IR::expression_type::binary_op:
		{
			expression->set_operand(0, apply_xchg(expression->get_operand(0), xchg_list));
...
{
	// fixme: some instruction shares pointer so xchg applied several times
...
void simplify_instructions(std::list<std::shared_ptr<IR::Instruction>>& instructions, bool basic_block)
{
	...
	{
		if (expression && expression->get_type() == IR::expression_type::variable)
		{
	...
	for (auto it = instructions.rbegin(); it != instructions.rend();)
	{
		...
		switch (instruction->get_id())
		{
			case IR::instruction_id::assign:
			{
				...
				{
					refcount[lhs] = 0;
					if (lhs->get_type() == IR::expression_type::memory)
					{

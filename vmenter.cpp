...
void ThemidaAnalyzer::lift_vm_enter(AbstractStream& stream, triton::uint64 address, vmenter_objects& ctx)
{
	...
	{
		const std::shared_ptr<x86_instruction> xed_inst = *it;
		const std::vector<xed_uint8_t> bytes = xed_inst->get_bytes();

		// fix ip
		triton_api->setConcreteRegisterValue(this->get_ip_register(), xed_inst->get_addr());

		// do stuff with triton
		triton::arch::Instruction triton_inst;
		triton_inst.setOpcode(&bytes[0], (triton::uint32)bytes.size());
		triton_inst.setAddress(xed_inst->get_addr());
		if (!triton_api->processing(triton_inst))
		{
			throw std::runtime_error("triton processing failed");
		}

		this->vmenter_check_lock_xchg(triton_inst, ctx);
		this->vmenter_check_eflags(triton_inst, ctx);
		this->vmenter_check_store_access(triton_inst, ctx);

		if (xed_inst->get_category() != XED_CATEGORY_UNCOND_BR
			|| xed_inst->get_branch_displacement_width() == 0)
		{
			//std::cout << "\t" << triton_inst << "\n";
		}
...
l_done:
	...
	for (const auto& pair : ctx.initial_data)
	{
		...
		if (_addr == ctx.lock_addr)
		{
			...
			memory_value == 0x400000
			//memory_value % 0x1000 == 0 // lazy check tho
			)
		{

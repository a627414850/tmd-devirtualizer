...
extern unsigned int deobfuscate_basic_block(std::shared_ptr<BasicBlock> basic_block);

extern std::shared_ptr<BasicBlock> make_cfg(AbstractStream& stream, unsigned long long address);

extern std::shared_ptr<BasicBlock> triton_make_cfg(AbstractStream& stream, uint64_t address);
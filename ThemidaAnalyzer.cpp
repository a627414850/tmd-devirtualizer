...
constexpr bool print_debug = 1;
constexpr bool print_warning = 1;
...
void ThemidaAnalyzer::vmhandler_check_store_access(triton::arch::Instruction &triton_instruction, std::shared_ptr<ThemidaHandlerContext> context)
{
	...
	for (const std::pair<triton::arch::MemoryAccess, triton::ast::SharedAbstractNode>& pair : storeAccess)
	{
		...
		if (address == context->bytecode_addr)
		{
			...
			if (print_debug) std::cout << "bytecode=0x" << std::hex << bytecode << '\n';
		...
		else if (is_context_address(lea_ast, context))
		{
			...
			{
				// VM_REG_X = Y
				if (print_debug) std::cout << "Store VM_REG [EBP+0x" << std::hex << offset << "]\n";
				...
				{
					// VM_REG_X = immediate
					std::shared_ptr<IR::Expression> lhs = this->get_vm_register(offset, mem.getSize()); // dont inc count here
					std::shared_ptr<IR::Instruction> _assign = std::make_shared<IR::Assign>(lhs, std::make_shared<IR::Immediate>(
						source_node->evaluate().convert_to<triton::uint64>()));
					context->instructions.push_back(std::move(_assign));
					continue;
				}
			...
			{
				if (print_debug) std::cout << "Store [EBP+0x" << std::hex << offset << "](STATIC)\n";
		...
		else if (is_stack_address(lea_ast, context))
		{
			// stores to stack
			triton::sint64 stack_offset = address - context->stack;
			if (0 <= stack_offset)
			{
				IR::exprptr expr;
				...
				else
				{
					...
					expr = context->triton_to_expr(symvar1);
				}
				...
				auto ir_stack_address = std::make_shared<IR::Add>(
					context->triton_to_expr(context->symvar_stack),
					std::make_shared<IR::Immediate>(stack_offset));
				auto variable = std::make_shared<IR::Memory>(ir_stack_address, seg_reg, mem.getSize());

				// seg:[sp+stack_offset] = expr
				context->instructions.push_back(std::make_shared<IR::Assign>(variable, expr));
			}
			else
			{
				// can be push or just local variable for a handler
			}
		}
		else
		{
			...
			if (symvar0 && expr)
			{
				...
				std::shared_ptr<IR::Expression> lhs = std::make_shared<IR::Memory>(context->triton_to_expr(symvar0), seg_reg, mem.getSize());
				context->instructions.push_back(std::make_shared<IR::Assign>(lhs, expr));
				context->runtime_memory[lhs] = address;
			}
			else
			{
				if (print_warning)
				{
					std::cout << "unknown store addr: " << std::hex << address
						<< ", lea_ast: " << lea_ast 
						<< ", simplified node: " << simplified_source_node << '\n';
				}
			}
		}
	}
		...
		|| triton_instruction.getType() == triton::arch::x86::ID_INS_PUSHFQ)
	{
		...
		auto eflags_ast = this->triton_api->getRegisterAst(triton_eflags);
		triton::engines::symbolic::SharedSymbolicVariable symvar_eflags = get_symbolic_var(triton_api->processSimplification(eflags_ast, true));
		if (symvar_eflags)
		{
			triton::arch::MemoryAccess _mem(this->get_sp(), triton_eflags.getSize());
			auto symvar_mem = triton_api->symbolizeMemory(_mem);
			context->m_expression_map[symvar_mem->getId()] = context->triton_to_expr(symvar_eflags->getId());
		}
	}
}
	...
	const std::vector<std::shared_ptr<IR::Expression>>& operands_expressions, hdlr_ctx_ptr context)
{
	...
	for (const auto& pair : triton_instruction.getWrittenRegisters())
	{
		...
		if (this->triton_api->isFlag(written_register))
		{
			...
			context->instructions.push_back(std::make_shared<IR::Assign>(_tvar_eflags, std::make_shared<IR::Flags>(make_copy(expr))));
...
void ThemidaAnalyzer::run_vm_handler(AbstractStream& stream, triton::uint64 handler_address, hdlr_ctx_ptr context)
{
	...
	for (auto it = basic_block->instructions.begin(); it != basic_block->instructions.end();)
	{
		...
		// disassembly and symbolize memory before "buildSemantics"
		...
		this->vmhandler_check_store_access(triton_instruction, context);
		...
		{
			expected_return_address = xed_instruction->get_addr() + xed_instruction->get_length();
		}
		...
		while (it == basic_block->instructions.end())
		{
			if (basic_block->next_basic_block && basic_block->target_basic_block)
			{
				...
				if (testing)
				{
					auto symvars = collect_variable_nodes(triton_api->getRegisterAst(this->get_ip_register()));
					for (auto symvar : symvars)
					{
						auto it = context->m_expression_map.find(get_symbolic_var(symvar)->getId());
						if (it != context->m_expression_map.end())
						{
							for (const auto& pair : this->m_vm_regs)
							{
								if (pair.second == it->second)
								{
									// vm register affects path?
									printf("%llx choke\n", xed_instruction->get_addr());
									getchar();
								}
							}
						}
...
void ThemidaAnalyzer::lift_vm_handler(AbstractStream& stream, triton::uint64 handler_address, hdlr_ctx_ptr ctx)
{
	...
	try
	{
		...
		{
			// possibilities: jcc_out / call / jmp_out / ret / vemit
			...
			// collect all pops/ret
			auto insts = this->lift_vm_exit(ctx);
			ctx->instructions.insert(ctx->instructions.end(), insts.begin(), insts.end());

			// next handler address is zero right?
			...
			printf("ret: %016llX %016llX\n", possible_vemit_addr, ctx->return_address);
	...
	std::set<IR::Label>& visit, vm_label_t& labels, hdlr_ctx_ptr handler_ctx)
{
	...
	if (handler_ctx->return_address != 0)
	{
		// call detected
		printf("call detected %llx\n", handler_ctx->return_address);
...
void ThemidaAnalyzer::print_output()
{
	...
	//
	auto is_push = [](IR::handler_instructions& instructions) -> bool
	{
		/*
		VM_REG_X = topofstack
		mov VM_REG_X(SP),ADD(VM_REG_X(SP), OFF)
		mov esp,Add(esp, OFF)
		*/
